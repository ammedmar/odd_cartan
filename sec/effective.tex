...
\begin{lemma}
	Let $G$ be a group, $C$ a bounded-below graded chain complex of free $\Z[G]$-modules with a basis, and $D$ a graded chain complex of $\Z[G]$-modules together with a $\Z$-linear endomorphism $h$ satisfying for some chain endomorphism $\varepsilon$ that:
	\begin{enumerate}
		\item $\bd \circ \, h + h \circ \bd = \id_C - \varepsilon$,
		\item $\varepsilon\circ h = 0$.
	\end{enumerate}
	Then, for any pair of $\Z[G]$-linear chain maps $f,g \colon C \to D$ with $\varepsilon \circ (f-g) = 0$, the $\Z[G]$-linear map $K \colon C \to D$ recursively defined on basis elements by
	\[
...
\begin{proof}
	...
	\begin{align*}
		(\bd K+ K\bd)(b)
		&= \bd h(f-g-K\bd)(b) + K\bd(b) \\
		&= (\id_C - \varepsilon - h\bd)(f-g - K\bd)(b) + K\bd(b).
	\end{align*}
	If $b$ is a cycle we are done since $\varepsilon(f-g) = 0$ by assumption, in particular if $b$ is of lowest degree.
	Since $\varepsilon h = 0$ we have $\varepsilon K \bd(b) \defeq \varepsilon h(f-g-K)(\bd b) = 0$ for any $b$.
	Additionally, by the induction assumption $\bd K (\bd b) = (f-g-K\bd)(\bd b) = (f-g)(\bd b)$ we have $h \bd (f-g - K\bd)(b) = 0$.
	Therefore, we are left with
	\begin{align*}
		(\bd K+ K\bd)(b) &=
		\id_C(f-g-K\bd)(b)+K\bd(b) \\&=
		(f-g)(b)
	\end{align*}
	as desired.
\end{proof}
