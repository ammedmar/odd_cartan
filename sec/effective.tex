...
For $r \in \N$, let $H_1^{(r)} \colon \chains\EE\cyc_r \to \chains\EE\sym_{2r}$ be given by
\[
H_1^{(r)}(\sigma_0,\dots,\sigma_n) =
\sum_{i=0}^n \ (-1)^i (f^{(r)}(\sigma_0) \tau, \dots, f^{(r)}(\sigma_i) \tau, g^{(r)}(\sigma_i), \dots, g^{(r)}(\sigma_n)).
\]
It can be easily verified that $H_1^{(r)}$ is a homotopy between $\chains \EE f^{(r)} \cdot \tau$ and $\chains \EE g^{(r)}$.
To check that it is $\cyc_r$-equivariant we use \cref{eq:conjugation of little maps} and the fact that $\rho$ acts on $\chains\EE\sym_{2r}$ by right multiplication by $g(\rho)$.
...
Given $K$ as above, the $\cyc$-module map $H_2$ given for each $r \in \N$ by
\[
H_2^r \colon \cW(p) \xra{K^r} \cE(p)^{\ot 2} \xra{\psi^2_0 \ot \id} \cE(2) \ot \cE(r)^{\ot 2} \xra{\bcirc_{\cE}} \cE(2r)
\]
...
\[
H_3^r \colon \cW(r) \xra{\psi(r)} \cE(r) \xra{\psi^2_0 \ot \Delta_{\mathrm{SHI}}} \cE(2) \ot \cE(r)^{\ot 2} \xra{\bcirc_{\cE}} \cE(2r)
\]
is a $\cyc_r$-equivariant chain homotopy between $\chains\EE g$ and $\widetilde G_\psi^p$.

Let $H_3^r$ be the $\cyc$-module map defined by these maps, which is a chain homotopy

\subsection{Main construction}

\begin{theorem}
	For any algebra over the Barratt--Eccles operad $\phi \colon \cE \to \End(A)$, the map $H = \phi \circ (H_1 + H_2 + H_3)$ is a Cartan relator for $(A,\psi\circ\iota)$.\todo{Maybe the signs are not right here.}
\end{theorem}
