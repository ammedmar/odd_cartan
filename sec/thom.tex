...
Additionally,\federico{I propose to change ''additionally'' for ''As before''} we write $({s_0},\dots,{s_n})$ instead of $(\rho^{s_0},\dots,\rho^{s_n})$ for elements in $\EE\cyc_p$.
...
\anibal{I see your point, but notice that the target of the map is in $\chains\EE\sym_p$, so at most we can change the first line.}