...
As before, we write $({s_0},\dots,{s_n})$ instead of $(\rho^{s_0},\dots,\rho^{s_n})$ for elements in $\EE\cyc_p$.
...
\end{align*}
where the sum iterates over all $s_1,\ldots,s_i \in \cyc_p \cong \set{0,\dots,p-1}$.
...
\begin{proof}
	We will write $L$ instead of $L_1$ to lighten the notation.
	In degree zero, our formula reduces to $L(e_{0}) = 0$, which agrees with $\eta\circ (\mu - \nu - L\circ \bd)$, because $\mu(e_0) = \nu(e_0)$ and $\bd (e_0) = 0$.
	For the induction step we distinguish two cases, depending on the parity of the degree.
	...
	Since the first entry of each summand in $\nu(e_{2i+1})$ is $0$, the term $\eta \circ \nu(e_{2i+1})$ vanishes.
	Notice that
	\[
	...
	Since $s_1,\ldots,s_i$ take all possible values, we can relabel $s_j+k$ as $s_j$ for each $j=1\ldots i$, obtaining
	\[
	L(\rho e_{2i}) \ = \
	-\sum_{\ell = 1}^i k^{i-\ell} \, \sum_{j=1}^{k-1} \, \sum \
	\begin{aligned}
...
\]
which, when restricted to $\chains\EE\cyc_p$, is equivariant with respect to the homomorphisms $\rho \mapsto \rho^k$ as the following computation verifies:
\begin{align*}
