...
Throughout this section we assume that $A$ is chain complex equipped with a May--Steenrod structure.
...
As we will explain, the existence of a Cartan relator implies the Cartan formulas on the mod $p$ homology of $A$, which are given by
\begin{align*}
	\rP_s\big([a][b]\big) =&
...
\begin{align*}
	...
	&C_\psi^p(2i+1)(a,b) = (-1)^{\floor{p/2}\bars{a}\bars{b}} \psi_{2i+1}\big((a \smallsmile b)^{\ot p}\big) \\
	&\quad -\sum_{i=j+k} \big(\psi_{2j+1}(a^{\ot p})\big) \smallsmile \big(\psi_{2k}(b^{\ot p})\big)\ +\
	(-1)^{\bars{a}}\big(\psi_{2j}(a^{\ot p})\big) \smallsmile \big(\psi_{2k+1}(b^{\ot p})\big).
\end{align*}
Then, if $a,b \in A \ot \Fp$ are cycles, the expression $C_\psi^p(i)(a,b)$ is a cycle representing the non-zero side of the Cartan formulas, which is an explicit boundary
\begin{align*}
